...
def parse_args():
    ...
    parser.add_argument(
        ...
        help='Path to VG vocab.json (for SG-conditioned prompts).')
    parser.add_argument(
        '--use_sg_prompt',
        action='store_true',
        help='If set, build BLIP prompts from scene-graph objects to reduce hallucinations.')
...
def build_allowed_objects_map(dataset_dir: Path, splits, vocab_json: Path):
    """
    Build a mapping from image relative path -> sorted list of object labels
    present in the VG scene graph for that image.
    """
    ...
    for split in splits:
        ...
        with h5py.File(split_path, 'r') as h5f:
            ...
            for idx in range(len(image_paths)):
                ...
                # Skip if we already collected labels from another split
...
def generate_captions(args):
    ...
    if args.use_sg_prompt:
        ...
        sg_object_map = build_allowed_objects_map(args.dataset_dir, args.splits, args.vocab_json)
        print(f'Found SG object lists for {len(sg_object_map)} images.')
    ...
    def process_batch(batch_images, batch_paths):
        ...
        if args.use_sg_prompt:
            ...
            for rel_path in batch_paths:
                labels = sg_object_map.get(rel_path, [])
                if labels:
                    ...
                    # Fallback generic prompt if no SG info available
